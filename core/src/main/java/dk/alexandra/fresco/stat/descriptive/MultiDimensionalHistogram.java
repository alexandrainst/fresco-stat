    ...
    return builder.par(par -> {
      ...
      MultiDimensionalArray<DRes<SInt>> histogram = MultiDimensionalArray
          .build(cumulativeHistogram.getWidths(), i -> par.seq(seq -> {
            ...
            List<Integer> nonZeroIndices = IntStream.range(0, dimensions).filter(d -> i.get(d) > 0).boxed()
                .collect(
                    Collectors.toList());
            if (!nonZeroIndices.isEmpty()) {
              subsets(nonZeroIndices).filter(subset -> !subset.isEmpty()).forEach(subset -> {
                List<Integer> x = IntStream.range(0, dimensions).map(d -> subset.contains(d) ? i.get(d) - 1 : i.get(d))
